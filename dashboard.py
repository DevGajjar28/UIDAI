...
def main():
    ...
    # Header
    st.markdown('<div class="main-header">📊 UIDAI Analytics Dashboard</div>', unsafe_allow_html=True)
    st.markdown("### AI-Driven Early Warning System for Aadhaar Update Surges and Anomalies")
    st.markdown("---")
    ...
    if 'daily' in data and len(data['daily']) > 0:
        ...
        if isinstance(date_range, tuple) and len(date_range) == 2:
            ...
            data['daily'] = data['daily'][
                (data['daily']['date'].dt.date >= start_date) & 
                (data['daily']['date'].dt.date <= end_date)
            ]
    
    # State filter
    if 'state' in data:
        all_states = ['All'] + sorted(data['state']['state'].unique().tolist())
